...
// Overriding this function from OOTB gigyaloginaddon.js to avoid redirecting to the homepage "..+ '/' " in all the scenarios after login success
// We want to redirect always to the referer
gigyaHybris.gigyaFunctions.raasLogin = function(response) {
    jQuery.ajax(ACC.config.contextPath + "/gigyaraas/login", {
        ...
        if (data.code !== 0) {
            ...
            // Redirecting to the ACC.config.contextPath instead of the homepage.
            // We could also redirect to the document.referrer, but on the checkout login scenario
            // that will redirect to the cart page instead of the checkout step 1 page
